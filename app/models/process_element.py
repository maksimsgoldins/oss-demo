...
from sqlalchemy import DateTime, Float, ForeignKey, JSON, String, UniqueConstraint, func, Column
...
class ProcessElement(Base):
    ...
    task_spec_id = Column(UUID(as_uuid=True), ForeignKey("task_specs.id"), nullable=True)
    gateway_spec_id = Column(UUID(as_uuid=True), ForeignKey("gateway_specs.id"), nullable=True)
    event_spec_id = Column(UUID(as_uuid=True), ForeignKey("event_specs.id"), nullable=True)
