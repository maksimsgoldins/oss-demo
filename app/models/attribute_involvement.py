...
class AttributeInvolvement(Base):
    ...
    __table_args__ = (UniqueConstraint("service_id", "attribute_id", name="uq_attribute_involvement"),)
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    service_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("services.id", ondelete="CASCADE"), nullable=False)
    attribute_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("attributes.id", ondelete="RESTRICT"), nullable=False)

class AttributeInvolvementAllowedValue(Base):
    __tablename__ = "attribute_involvement_allowed_values"
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
