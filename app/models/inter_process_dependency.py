...
from sqlalchemy import DateTime, ForeignKey, JSON, String, Text, func, Column
...
class InterProcessTaskDependency(Base):
    ...
    source_process_spec_id = Column(UUID(as_uuid=True), ForeignKey("process_specs.id"), nullable=False, index=True)
    source_element_id = Column(UUID(as_uuid=True), ForeignKey("process_elements.id"), nullable=False, index=True)
    target_process_spec_id = Column(UUID(as_uuid=True), ForeignKey("process_specs.id"), nullable=False, index=True)
    target_element_id = Column(UUID(as_uuid=True), ForeignKey("process_elements.id"), nullable=False, index=True)
    service_relation_id = Column(UUID(as_uuid=True), ForeignKey("service_relations.id"), nullable=True, index=True)
