...
from app.models.attribute_involvement import (
    AttributeInvolvement,
    AttributeInvolvementAllowedValue,
    AttributeInvolvementDefaultValue,
)
from app.models.service import Service
from app.schemas.attribute_involvement import (
    AttributeInvolvementCreate,
    AttributeInvolvementRead,
    AttributeInvolvementUpdate,
)

router = APIRouter(prefix="/api/attribute-involvement", tags=["attribute-involvement"])

def base_possible_values(db: Session, attribute_id):
    return [
        x.value_text
        for x in db.execute(
...
def validate_sets(attr: Attribute, base_values: list[str], allowed_values: list[str], default_values: list[str]):
    allowed_values = allowed_values or []

    if base_values:
        effective_allowed = allowed_values or base_values
        invalid_allowed = [v for v in effective_allowed if v not in base_values]
        if invalid_allowed:
            raise HTTPException(status_code=400, detail=f"Invalid allowed values: {invalid_allowed}")

        invalid_default = [v for v in default_values if v not in effective_allowed]
        if invalid_default:
            raise HTTPException(status_code=400, detail=f"Invalid default values: {invalid_default}")
    else:
        if allowed_values:
            raise HTTPException(status_code=400, detail="Allowed values cannot be set when attribute has no configured possible values")
        effective_allowed = []
...
def serialize(db: Session, item: AttributeInvolvement):
    allowed = db.execute(
        select(AttributeInvolvementAllowedValue)
        .where(AttributeInvolvementAllowedValue.attribute_involvement_id == item.id)
        .order_by(AttributeInvolvementAllowedValue.sort_order)
    ).scalars().all()
    defaults = db.execute(
        select(AttributeInvolvementDefaultValue)
        .where(AttributeInvolvementDefaultValue.attribute_involvement_id == item.id)
        .order_by(AttributeInvolvementDefaultValue.sort_order)
    ).scalars().all()
...
def create_attribute_involvement(payload: AttributeInvolvementCreate, db: Session = Depends(get_db)):
    ...
    exists = db.execute(
        select(AttributeInvolvement).where(
            AttributeInvolvement.service_id == payload.service_id,
            AttributeInvolvement.attribute_id == payload.attribute_id,
        )
    ).scalar_one_or_none()
    if exists:
