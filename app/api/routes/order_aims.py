...
def list_order_aims(db: Session = Depends(get_db)):
    ...
    res=[]
    for aim in aims:
        subs = db.execute(select(OrderSubAim).where(OrderSubAim.order_aim_id==aim.id).order_by(OrderSubAim.code)).scalars().all()
        res.append(OrderAimRead(id=aim.id, code=aim.code, name=aim.name, sub_aims=[OrderSubAimItem(code=s.code, name=s.name) for s in subs]))
    return res
